...
function(stm32_generate_dis_file TARGET)
    get_target_property(TARGET_OUTPUT_NAME ${TARGET} OUTPUT_NAME)
    if (TARGET_OUTPUT_NAME)
        set(OUTPUT_FILE_NAME "${TARGET_OUTPUT_NAME}.dis")
    else()
        set(OUTPUT_FILE_NAME "${TARGET}.dis")
    endif()
    ...
    add_custom_command(
        TARGET ${TARGET}
        POST_BUILD
        COMMAND ${CMAKE_OBJDUMP} -h $<TARGET_FILE:${TARGET}> > ${OUTPUT_FILE_PATH}
        COMMAND ${CMAKE_OBJDUMP} -d $<TARGET_FILE:${TARGET}> >> ${OUTPUT_FILE_PATH}
        BYPRODUCTS ${OUTPUT_FILE_PATH}
        COMMENT "Generating disassembly file ${OUTPUT_FILE_NAME}"
    )
endfunction()

function(stm32_generate_extra_files TARGET)
    stm32_generate_hex_file(${TARGET})
    stm32_generate_binary_file(${TARGET})
    stm32_generate_dis_file(${TARGET})
    stm32_generate_map_file(${TARGET})
endfunction()
